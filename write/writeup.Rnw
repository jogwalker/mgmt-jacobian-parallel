...
% # <<another-table, cache=TRUE,echo=FALSE,results='hide', warning=FALSE>>=
% # # gib's fun colorful thing
% # 
% # require(ggplot2, quietly=TRUE)
% # require(reshape2. quietly=TRUE)
% # a <- melt(netmat)
% # levels(a$value) <- c("Abiotic", "Competition", "Density dependent", "Incidental", "Mutualistic", "None",
% # "Predator", "Prey", "Unknown")
% # temp <- as.character(unique(a$Var1))
% # a$Var1 <- factor(a$Var1, levels=temp)
% # a$Var2 <- factor(a$Var2, levels=temp)
% # ggplot(a, aes(x=1, y=1, fill=value)) + geom_tile() + scale_fill_brewer(type="qual",palette="Set3") + facet_grid(Var1 ~ Var2) 
% # 
% # @

<<methods-1,cache=TRUE, echo=FALSE >>=
...
Instead of taking the approach  of \citet{Raymond2011}, which leads to equal weighting of each combination regardless of the likelihood of that combination yielding a stable matrix, I use a two step approach to simulate unknown values. For each iteration of the network, I first sample a number from a uniform distribution between 0 and 1 to determine the proportion of unknown links that are 0, this is used as p in a binomial distribution. This binomial distribution is sampled for each of the unknown links, those that are chosen to be non-zero then have a value simulated from a gamma(0.5,1) distribution, and half of the non-zero values are randomly chosen to be negative (by sampling from another binomial distribution).  Various changes to how each $a_{ij}$ is chosen makes it more or less likely that a simulated \textbf{A} will be stable, but in general many iterations are necessary to reach a large number of stable simulations. At the moment all random values $a_{ij}$ are drawn from a gamma(-.5,1) distribution, which means $|a_{ij}|$ is most likely to be $ < 1 $.
