% rewriting from 27 June 2013 - copied file over from write directory
...
The first step is to develop a conceptual network model of the ecosystem based on Lotka-Volterra interactions, incorporating uncertain links. Next, I subject simulations of this network to combinations of press perturbations which represent management strategies. The results of the press perturbations indicate whether certain nodes in the network are likely to increase or decrease following the perturbation. I plan to conduct interviews with communities and government officials in Botswana to estimate how certain aspects of the system or certain nodes are valued compared to others. I will then incorporate these results into cost-benefit equations that incorporate implications for conservation as well as the needs of the communities. In this way, management strategies can be compared based on their predicted cost-benefit ratio, as well as the uncertainty of the outcome for each node. 
...
The system is linearized by evaluating the first partial derivatives of the growth functions $f_{i}$ with respect to each $X_{j}$ at equilibrium, this makes up the $n\times n$ Jacobian matrix \textbf{A}.
...
%\paragraph{}
The values of each individual parameter $p_{k}$ in the Lotka-Volterra equations or each $a_{ij}$ in \textbf{A} are not known, but the results of the model are likely to be highly sensitive to the values chosen \citep{Hosack2008}. Instead of defining values, I defined the biological class of interaction for each cell in the matrix (for example: predator, prey, competition) in order to decide whether $a_{ij}$ will be positive or negative  
% (Table \ref{tab:network}). 
This is one sense in which the model is 'qualitative'.

<<setup, cache=TRUE,warning=FALSE, echo=FALSE,results='hide'>>=
require(xtable)
load("O:/Bristol_PhD/R/mgmt-jacobian-parallel/network.rdata")
#load("~/Bristol_PhD/R/mgmt-jacobian-parallel/network.rdata")
...
% <<another-table, cache=TRUE,echo=FALSE, figs.only=TRUE , warning=FALSE>>=
% # gib's fun colorful thing
% require(ggplot2, quietly=TRUE)
% require(reshape2, quietly=TRUE)
% a <- melt(netmat)
% levels(a$value) <- c("Abiotic", "Competition", "Density dependent", "Incidental", "Mutualistic", "None","Predator", "Prey", "Unknown")
% temp <- as.character(unique(a$Var1))
% a$Var1 <- factor(a$Var1, levels=temp)
% a$Var2 <- factor(a$Var2, levels=temp)
% ggplot(a, aes(x=1, y=1, fill=value)) + 
%   geom_tile() + 
...
<<methods-1,echo=FALSE, results='asis'>>=
print(xtable(network, caption="Defined interactions between nodes; ab = abiotic/incidental increase, uk = unknown, mu = mutualistic, pr = predator, py = prey, co = competition, dd = density-dependent (diagonal), na = none, in = incidental reduction", label="tab:network"), size="footnotesize", include.colnames=TRUE,
...
Instead of taking the approach  of \citet{Raymond2011}, which leads to equal weighting of each combination regardless of the likelihood of that combination yielding a stable matrix, I use a two step approach to simulate unknown values. For each iteration of the network, I first sample a number from a uniform distribution between 0 and 1 to determine the proportion of unknown links that are 0, this is used as p in a binomial distribution. This binomial distribution is sampled for each of the unknown links, those that are chosen to be non-zero then have a value simulated from a gamma(0.5,1) distribution. Half of the non-zero values are randomly chosen to be negative (by sampling from another binomial distribution).   At the moment all random values $a_{ij}$ are drawn from a gamma(.5,1) distribution, which means $|a_{ij}|$ is most likely to be $ < 1 $. Various changes to how each $a_{ij}$ is chosen makes it more or less likely that a simulated \textbf{A} will be stable, but in general many iterations are necessary to reach a large number of stable simulations. After 100 million simulations, only a few thousand are stable.

\begin{figure}
\begin{center}
<<gamma,echo=TRUE>>=
hist(rgamma(1000,0.5,1))
@
\end{center}
\caption{Histogram of Gamma distribution used for simulations}
\label{fig:gamma}
\end{figure}
